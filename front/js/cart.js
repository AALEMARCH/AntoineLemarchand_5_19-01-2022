...
//Mise en place des données nécessaires à l'envoi vers le serveur
...
commandButton.addEventListener("click", (event) => {
  ...
  if (
    ...
    lastNameRegex.test(contact.lastName) &&
    adressRegex.test(contact.address) &&
    cityRegex.test(contact.city) &&
      ...
      async (res) => {
        try {
          if (res.ok) {
            ...
            alert(`Problème avec le serveur : erreur ${res.status}`);
          }
        } catch (error) {
